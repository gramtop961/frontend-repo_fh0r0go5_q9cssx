...
export default function Team() {
  return (
    <section id="team" className="relative bg-black text-white py-24">
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between gap-6">
          <div>
            <h2 className="text-3xl sm:text-4xl font-semibold tracking-tight">Executive Team</h2>
            <p className="mt-2 text-white/70 max-w-2xl">A small team with a big bias for action.</p>
          </div>
        </div>

        <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {team.map((m) => (
            <article key={m.name} className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] p-6 transition hover:border-emerald-400/40 hover:bg-white/[0.06]">
              ...
              <div className="mt-4 flex items-center gap-3 text-sm text-white/70">
                <a href={m.linkedin} className="inline-flex items-center gap-1 hover:text-white transition" aria-label="LinkedIn">
                  <Linkedin size={16} />
                  LinkedIn
                </a>
                <a href={`mailto:${m.email}`} className="inline-flex items-center gap-1 hover:text-white transition" aria-label="Email">
                  <Mail size={16} />
                  Email
                </a>
              </div>
